...
function defaultData() {
      ...
      try {
        ...
        if (raw) {
          ...
          if (p && typeof p === "object") {
            ...
            /* 自愈：清理非法 key、补齐缺失功能、保证选中项合法（防收纳架等误改带崩生活区） */
            healLife();
            /* 旧模块配色迁移到莫兰迪淡绿/青色系 */
  ...
  function renderHomeCountdown() {
    ...
    function itemHtml(item, idx) {
      ...
      /* 自动配色：≤15 天珊瑚红渐变，≤30 天西柚橙渐变，铺满整卡；无手动色时保留默认左条 */
      var gradClass = "", boxStyle = "";
      if (diff > 0 && diff <= 15) {
        gradClass = " cd-grad-red";
        boxStyle = ' style="background:linear-gradient(135deg,#F4B5B5 0%,#E58A8A 100%);border-left-color:transparent;"';
      } else if (diff > 0 && diff <= 30) {
        gradClass = " cd-grad-orange";
        boxStyle = ' style="background:linear-gradient(135deg,#FFD9C8 0%,#F7A46D 100%);border-left-color:transparent;"';
      } else if (item.color) {
        boxStyle = ' style="border-left:3px solid ' + esc(item.color) + '"';
      }
      return '<div class="cd-item ' + statusClass + gradClass + '" ' + boxStyle + ' data-cd-idx="' + idx + '">'
  ...
  /* ===================== 娱乐区·文包目录（单本 / 文包） ===================== */
  var ArchiveParser = (function () {
    var NOVEL_EXT = /\.(txt|epub|mobi|pdf|azw3|docx?|md)$/i;
    var PKG_EXT = /\.(zip|dzip|tar|tgz|gz|iso|7z|rar)$/i;
    function fixDzipSig(u8) {
      var out = u8.slice();
      for (var i = 0; i + 3 < out.length; i++) {
        if (out[i] === 0x55 && out[i+1] === 0x41 && out[i+2] === 0x03 && out[i+3] === 0x04) { out[i] = 0x50; out[i+1] = 0x4B; }
        else if (out[i] === 0x55 && out[i+1] === 0x41 && out[i+2] === 0x05 && out[i+3] === 0x06) { out[i] = 0x50; out[i+1] = 0x4B; }
        else if (out[i] === 0x55 && out[i+1] === 0x41 && out[i+2] === 0x01 && out[i+3] === 0x02) { out[i] = 0x50; out[i+1] = 0x4B; }
      }
      return out;
    }
    function inflateRaw(slice) {
      var ds = new DecompressionStream('deflate-raw');
      var w = ds.writable.getWriter(); w.write(slice); w.close();
      return new Response(ds.readable).arrayBuffer().then(function (b) { return new Uint8Array(b); })
        .catch(function () {
          ...
          var w2 = ds2.writable.getWriter(); w2.write(slice); w2.close();
          return new Response(ds2.readable).arrayBuffer().then(function (b) { return new Uint8Array(b); });
        });
    }
    function decodeBytes(bytes) {
      try { var s = new TextDecoder('utf-8', { fatal: true }).decode(bytes); if (!s.includes('�')) return s; } catch (e) {}
      try { return new TextDecoder('gbk').decode(bytes); } catch (e) {}
      try { return new TextDecoder('big5').decode(bytes); } catch (e) {}
      return new TextDecoder('iso-8859-1').decode(bytes);
    }
    function readStr(u8, off, len) { var end = off; while (end < off + len && u8[end] !== 0) end++; return decodeBytes(u8.subarray(off, end)); }
    function parsePax(content) {
      var m = {}; var i = 0;
      while (i < content.length) {
        var j = i; while (j < content.length && content[j] >= '0' && content[j] <= '9') j++;
        var len = parseInt(content.substring(i, j), 10); if (!len) break;
        var line = content.substring(j, j + len);
        var sp = line.indexOf(' '); var kv = sp >= 0 ? line.substring(sp + 1) : line;
        var eq = kv.indexOf('=');
        if (eq > 0) m[kv.substring(0, eq)] = kv.substring(eq + 1, kv.length - 1);
        i = j + len;
      }
      return m;
    }
    function parseZipDeep(u8) {
      var dv = new DataView(u8.buffer, u8.byteOffset, u8.byteLength);
      var eocd = -1;
      for (var i = u8.length - 22; i >= 0; i--) { if (u8[i] === 0x50 && u8[i+1] === 0x4B && u8[i+2] === 0x05 && u8[i+3] === 0x06) { eocd = i; break; } }
      if (eocd < 0) return null;
      var cdOffset = dv.getUint32(eocd + 16, true);
      var cdCount = dv.getUint16(eocd + 10, true);
      var out = []; var p = cdOffset;
      for (var n = 0; n < cdCount; n++) {
        ...
        var fnStart = p + 46;
        var bytes = u8.subarray(fnStart, fnStart + fnLen);
        var name = (flag & 0x0800) ? new TextDecoder('utf-8', { fatal: false }).decode(bytes) : decodeBytes(bytes);
        ...
        out.push({ name: name, isDir: /\/$/.test(name), method: method, dataStart: dataStart, compSize: compSize });
        p = fnStart + fnLen + exLen + cmLen;
      }
      return out;
    }
    function parseZipLocal(u8) {
      var out = []; var p = 0;
      var dv = new DataView(u8.buffer, u8.byteOffset, u8.byteLength);
      while (p + 4 <= u8.length) {
        if (u8[p] !== 0x50 || u8[p+1] !== 0x4B || u8[p+2] !== 0x03 || u8[p+3] !== 0x04) { p++; continue; }
        if (p + 30 > u8.length) break;
        var flag = dv.getUint16(p + 6, true);
        var method = dv.getUint16(p + 8, true);
        var compSize = dv.getUint32(p + 18, true);
        var fnLen = dv.getUint16(p + 26, true);
        var exLen = dv.getUint16(p + 28, true);
        var fnStart = p + 30;
        var nameBytes = u8.subarray(fnStart, fnStart + fnLen);
        var name = (flag & 0x0800) ? new TextDecoder('utf-8', { fatal: false }).decode(nameBytes) : decodeBytes(nameBytes);
        var dataStart = fnStart + fnLen + exLen;
        var isDir = /\/$/.test(name);
        var adv = (compSize > 0) ? compSize : 0;
        out.push({ name: name, isDir: isDir, method: method, dataStart: dataStart, compSize: compSize });
        p = dataStart + adv;
        if (adv === 0) p = Math.max(p, fnStart + fnLen + exLen + 1);
        if (p <= 0) break;
      }
      return out;
    }
    function parseTarEntries(u8) {
      var out = []; var p = 0; var B = 512; var pendingLong = null; var paxPath = null; var paxSize = null;
      while (p + B <= u8.length) {
        var allZero = true; for (var k = 0; k < B; k++) { if (u8[p + k] !== 0) { allZero = false; break; } }
        if (allZero) break;
        var name = readStr(u8, p, 100).replace(/\0+$/, '');
        var sizeStr = readStr(u8, p + 124, 12).replace(/\0+$/, '');
        var size = parseInt(sizeStr.trim() || '0', 8) || 0;
        var typeflag = String.fromCharCode(u8[p + 156]);
        var dataStart = p + B;
        if (typeflag === 'L') { pendingLong = readStr(u8, dataStart, size).replace(/\0+$/, ''); p = dataStart + B * Math.ceil(size / B); continue; }
        if (typeflag === 'x' || typeflag === 'g') {
          var content = new TextDecoder('utf-8').decode(u8.subarray(dataStart, dataStart + size));
          var pax = parsePax(content);
          if (pax.path) paxPath = pax.path; if (pax.size) paxSize = parseInt(pax.size, 10);
          p = dataStart + B * Math.ceil(size / B); continue;
        }
        var finalName = pendingLong !== null ? pendingLong : (paxPath !== null ? paxPath : name); pendingLong = null; paxPath = null;
        if (paxSize !== null) { size = paxSize; paxSize = null; }
        var isDir = typeflag === '5' || /\/$/.test(finalName);
        if (finalName) out.push({ name: finalName, isDir: isDir, method: 0, dataStart: dataStart, compSize: size });
        p = dataStart + B * Math.ceil(size / B);
      }
      return out;
    }
    function sniff(name, u8) {
      var low = name.toLowerCase();
      var isZip = u8[0] === 0x50 && u8[1] === 0x4B && u8[2] === 0x03 && u8[3] === 0x04;
      var isDzip = u8[0] === 0x55 && u8[1] === 0x41 && u8[2] === 0x03 && u8[3] === 0x04;
      if (low.endsWith('.dzip') || isDzip) return 'dzip';
      if (low.endsWith('.zip') || isZip) return 'zip';
      if (low.endsWith('.tar')) return 'tar';
      if (low.endsWith('.iso')) return 'iso';
      return null;
    }
    function collect(u8, name, depth, out, prefix) {
      if (depth > 5) return Promise.resolve();
      var kind = sniff(name, u8);
      var entries = null;
      if (kind === 'dzip') entries = parseZipDeep(fixDzipSig(u8));
      else if (kind === 'zip') entries = parseZipDeep(u8);
      else if (kind === 'tar') entries = parseTarEntries(u8);
      else if (kind === 'iso') {
        var fixed = (u8[0] === 0x55 && u8[1] === 0x41) ? fixDzipSig(u8) : u8;
        entries = parseZipDeep(fixed);
        if (!entries || !entries.length) entries = parseTarEntries(u8);
      }
      if ((!entries || !entries.length) && (kind === 'zip' || kind === 'dzip' || kind === 'iso')) entries = parseZipLocal(u8);
      if (!entries || !entries.length) return Promise.resolve();
      var chain = Promise.resolve();
      entries.forEach(function (en) {
        if (en.isDir || !en.name) return;
        var full = en.name;
        var base = full.split('/').pop();
        var innerDir = full.indexOf('/') >= 0 ? full.slice(0, full.lastIndexOf('/')) : '';
        /* 把当前压缩包/文件夹名作为前缀，保留原始文件夹结构（包括嵌套压缩包名） */
        var dir = prefix ? (innerDir ? prefix + '/' + innerDir : prefix) : innerDir;
        var series = dir.split('/')[0] || '';
        if (NOVEL_EXT.test(base) || /《[^》]{1,60}》/.test(base)) out.push({ name: full, base: base, dir: dir, series: series });
        else if (PKG_EXT.test(base)) {
          /* 关键：子压缩包名必须拼进前缀，否则子包内文件会与外层文件夹同名分组混淆 */
          chain = chain.then(function () {
            var content = en.method === 0 ? u8.subarray(en.dataStart, en.dataStart + en.compSize) : null;
            var nextPrefix = (dir ? dir + '/' : '') + base;
            if (content && content.length) return collect(content, base, depth + 1, out, nextPrefix);
            return inflateRaw(u8.subarray(en.dataStart, en.dataStart + en.compSize)).then(function (c) { if (c && c.length) return collect(c, base, depth + 1, out, nextPrefix); });
          });
        }
      });
      return chain;
    }
    function readPackNames(file) {
      return file.arrayBuffer().then(function (buf) {
        var u8 = new Uint8Array(buf); var out = [];
        return collect(u8, file.name, 0, out, '').then(function () { return out; });
      });
    }
    return { readPackNames: readPackNames };
  })();
  ...
  function titleFromFilename(base) {
    ...
    n = n.replace(/(by|作者|—)\s*[:：]?\s*[一-龥A-Za-z0-9_·]{1,30}$/i, '').trim();
    return n || '待整理';
  }
  function authorFromFilename(base) {
    var m = (base || '').match(/(?:by|作者|—)\s*[:：]?\s*([一-龥A-Za-z0-9_·]{1,30})/i);
    return m ? m[1].trim().split(/[（(【]/)[0].trim() : '';
  }
  function parseSingleInput(text) {
    text = (text || '').trim();
    var r = { title: '', author: '', source: '', code: '', isBaidu: false };
    var bM = text.match(/https?:\/\/pan\.baidu\.com\/s\/[A-Za-z0-9_-]+/);
    if (bM) {
      r.isBaidu = true; r.source = bM[0];
      var cM = text.match(/提取码[:：]?\s*([A-Za-z0-9]{4})/i) || text.match(/验证码[:：]?\s*([A-Za-z0-9]{4})/i) || text.match(/pwd[=:]?\s*([A-Za-z0-9]{4})/i);
      if (cM) {
        r.code = cM[1];
        /* 把提取码直接拼进链接，复制/点击即可直达，不用手动输入 */
        if (r.source.indexOf('?') < 0) r.source += '?pwd=' + r.code;
        else if (!/[?&]pwd=/.test(r.source)) r.source += '&pwd=' + r.code;
      }
    }
    var tM = text.match(/《([^》\n]{1,60})》/);
    if (tM) r.title = tM[1].trim();
    var aM = text.match(/(?:by|作者|—)\s*[:：]?\s*([一-龥A-Za-z0-9_·]{1,30})/i);
    if (aM) r.author = aM[1].trim().split(/[（(【]/)[0].trim();
    if (!r.title) {
      var pre = text.split(/链接|pan\.baidu/i)[0].trim();
      var cM2 = pre.match(/[一-龥][一-龥A-Za-z0-9_·]{1,}/);
      r.title = (cM2 && cM2[0] !== r.author) ? cM2[0] : '待整理';
    }
    return r;
  }
  var archSub = "single";
  function renderArchive() {
    if (archSub === "single") renderSingleList(); else renderArchiveList();
  }
  function starHtml(n) {
    n = n || 0; var h = '';
    for (var i = 1; i <= 5; i++) h += '<span class="star ' + (i <= n ? 'on' : '') + '" data-star="' + i + '">★</span>';
    return h;
  }
  function renderSingleList() {
    var ul = $("single-list"); if (!ul) return; ul.innerHTML = "";
    var list = Store.data.ent.single;
    if (!list.length) { ul.innerHTML = '<li class="hint" style="text-align:center;padding:20px 0">从百度网盘复制链接粘进来，先收着判断要不要保留</li>'; return; }
    list.forEach(function (s) {
      if (!("star" in s)) s.star = (s.keep === 'like' ? 5 : s.keep === 'neutral' ? 3 : 0);
      var li = document.createElement("li"); li.className = "novel-card single-card";
      var star = s.star || 0;
      var expanded = !!s._expanded;
      var sourceLink = '';
      if (s.source) {
        sourceLink = s.isBaidu
          ? '<a class="single-source-link" href="' + esc(s.source) + '" target="_blank" rel="noopener">（百度网盘） ' + esc(s.source) + '</a>'
          : '<div class="single-source-link">' + esc(s.source) + '</div>';
      }
      li.innerHTML =
        '<div class="single-head" data-act="toggle">' +
          '<span class="single-chev">' + (expanded ? '▼' : '▶') + '</span>' +
          '<span class="single-title">《' + esc(s.title || '待整理') + '》</span>' +
          (s.author ? '<span class="single-author">作者：' + esc(s.author) + '</span>' : '') +
          '<span class="single-starsum">' + starHtml(star) + '</span>' +
          '<button class="single-head-del" data-act="del" title="删除收录">×</button>' +
        '</div>' +
        '<div class="single-body" style="display:' + (expanded ? 'block' : 'none') + '">' +
          (sourceLink ? '<div class="single-row">' + sourceLink + '</div>' : '') +
          (s.code ? '<div class="single-row">提取码：' + esc(s.code) + '</div>' : '') +
          '<div class="single-stars" data-role="stars" style="margin:8px 0">点星星评分：' + starHtml(star) + '</div>' +
          '<div class="it-actions"><button data-act="del" class="del">删除</button></div>' +
        '</div>';
  ...
  function renderArchiveList() {
    var box = $("archive-list"); if (!box) return; box.innerHTML = "";
    if (!Store.data.ent) Store.data.ent = {};
    if (!Array.isArray(Store.data.ent.archive)) Store.data.ent.archive = [];
    var packs = Store.data.ent.archive;
    if (!packs.length) { box.innerHTML = '<div class="hint" style="text-align:center;padding:20px 0">还没有归档的文包，选一个压缩包试试</div>'; return; }
    packs.forEach(function (p) {
      var groups = {};
      /* 分组策略：优先把内部压缩包名作为分组名；如果没找到压缩包名，再取第一层文件夹名。
         这样可以穿透多层冗余文件夹，确保 zip/tar/dzip/iso 子包都能独立显示。 */
      var PKG_RE = /(^|\/)([^\/]+\.(zip|dzip|tar|iso|rar|7z|tgz|gz))(?=\/|$)/i;
      (p.items || []).forEach(function (it) {
        var rawDir = it.dir || '';
        var d = '散落文件';
        if (rawDir) {
          var m = rawDir.match(PKG_RE);
          if (m) d = m[2];
          else d = (rawDir.split('/')[0] || '散落文件').replace(/\/$/, '');
        }
        if (!groups[d]) groups[d] = [];
        groups[d].push(it);
      });
      var groupKeys = Object.keys(groups);
      var itemHtml = function (it) { return '<div class="pack-item">《' + esc(it.title || '待整理') + '》' + (it.author ? ' · ' + esc(it.author) : '') + '</div>'; };
      var hasOld = (p.items || []).some(function (it) { return it.dir === undefined; });
      var oldHint = hasOld ? '<div class="pack-old-hint">💡 这是旧版平铺数据。点击下方「重新解析此包」，选中同一个压缩包，就能按内部压缩包/文件夹重新分组。</div>' : '';
      var bodyHtml;
      if (groupKeys.length === 1 && groupKeys[0] === '散落文件') {
        bodyHtml = oldHint + '<div class="pack-folder">' +
          '<div class="pack-folder-name" data-fold="' + p.id + '-root"><span class="folder-chev">▼</span>📁 散落文件</div>' +
          '<div class="pack-folder-items" id="fold-' + p.id + '-root">' + groups['散落文件'].map(itemHtml).join('') + '</div>' +
          '</div>';
      } else {
        bodyHtml = oldHint + groupKeys.map(function (d, idx) {
          var foldId = p.id + '-' + idx;
          return '<div class="pack-folder">' +
            '<div class="pack-folder-name" data-fold="' + foldId + '"><span class="folder-chev">▼</span>📁 ' + esc(d) + '</div>' +
            '<div class="pack-folder-items" id="fold-' + foldId + '">' + groups[d].map(itemHtml).join('') + '</div>' +
            '</div>';
        }).join('');
      }
      var card = document.createElement("div"); card.className = "novel-card pack-card";
      card.innerHTML = '<div class="pack-head" data-toggle="' + p.id + '">' +
        '<span class="pack-name">📦 ' + esc(p.name) + '</span>' +
        '<span class="pack-count">(' + p.items.length + ' 本)</span>' +
        '<button class="pack-head-refresh" data-act="reparse" data-id="' + p.id + '" title="重新解析此包">↻</button>' +
        '<button class="pack-head-del" data-act="delpack" data-id="' + p.id + '" title="删除整个文包">×</button>' +
        '<span class="chev">▼</span>' +
        '</div>' +
        '<div class="pack-body" id="pack-' + p.id + '">' + bodyHtml +
        '<div class="it-actions" style="margin-top:12px"><button data-act="reparse" data-id="' + p.id + '" class="btn-secondary">重新解析此包</button></div></div>';
      box.appendChild(card);
    });
    box.querySelectorAll('[data-toggle]').forEach(function (t) {
      t.onclick = function () {
        var id = t.getAttribute('data-toggle');
        var body = $("pack-" + id); if (!body) return;
    ...
    box.querySelectorAll('[data-fold]').forEach(function (t) {
      t.onclick = function (e) {
        e.stopPropagation();
        var fid = t.getAttribute('data-fold');
        var items = $('fold-' + fid); if (!items) return;
        var chev = t.querySelector('.folder-chev');
        if (items.style.display === 'none') { items.style.display = 'flex'; if (chev) chev.textContent = '▼'; }
        else { items.style.display = 'none'; if (chev) chev.textContent = '▶'; }
      };
    });
    box.querySelectorAll('[data-act="delpack"]').forEach(function (b) {
      b.onclick = function (e) {
        e.stopPropagation();
        var id = b.getAttribute('data-id');
        var nm = (Store.data.ent.archive.filter(function (x) { return x.id === id; })[0] || {}).name || '';
        confirmDelete('删除文包', '删除《' + nm + '》的归档？', function () {
          Store.data.ent.archive = Store.data.ent.archive.filter(function (x) { return x.id !== id; });
          Store.save(); renderArchiveList();
        });
      };
    });
    box.querySelectorAll('[data-act="reparse"]').forEach(function (b) {
      b.onclick = function (e) {
        e.stopPropagation();
        var id = b.getAttribute('data-id');
        var inp = document.createElement('input');
        inp.type = 'file';
        inp.accept = '.zip,.tar,.iso,.dzip,.rar,.7z,.tgz,.gz';
        inp.onchange = function () {
          var f = this.files && this.files[0]; if (!f) return;
          toast('正在重新解析「' + f.name + '」…');
          ArchiveParser.readPackNames(f).then(function (items) {
            if (!items || !items.length) { toast('「' + f.name + '」里没找到小说文件（txt/epub…）'); return; }
            var its = items.map(function (it) {
              return { name: it.name, base: it.base, dir: it.dir, series: it.series, title: titleFromFilename(it.base), author: authorFromFilename(it.base) };
            });
            var arr = Store.data.ent.archive;
            for (var i = 0; i < arr.length; i++) { if (arr[i].id === id) { arr[i].items = its; arr[i].size = f.size; break; } }
            Store.save(); renderArchiveList();
            toast('已重新解析「' + f.name + '」，发现 ' + its.length + ' 本（含内部压缩包分组）');
          }).catch(function (err) {
            console.error(err);
            toast('解析失败：' + (err && err.message ? err.message : '压缩包格式可能不支持'));
          });
        };
        inp.click();
      };
  ...
  function archiveReverseSearch(q) {
    q = (q || '').trim().toLowerCase();
    var box = $("archive-search-result"); if (!box) return;
    if (!q) { box.innerHTML = ''; return; }
    if (!Store.data.ent) Store.data.ent = {};
    if (!Array.isArray(Store.data.ent.archive)) Store.data.ent.archive = [];
    var hits = [];
    Store.data.ent.archive.forEach(function (p) {
      p.items.forEach(function (it) {
        var hay = [it.title, it.author, it.base].join(' ').toLowerCase();
        if (hay.indexOf(q) >= 0) hits.push({ pack: p.name, item: it });
      });
    });
    if (!hits.length) { box.innerHTML = '<div class="hint" style="text-align:center;padding:12px 0">没有找到匹配的书籍</div>'; return; }
    box.innerHTML = hits.map(function (h) {
      return '<div style="font-size:12px;color:#5a7a5f;padding:8px;background:#f9fbf8;border-radius:8px;margin-bottom:4px">《' + esc(h.item.title || '待整理') + '》' + (h.item.author ? ' · ' + esc(h.item.author) : '') + ' <span style="color:#8aa885">→ 在《' + esc(h.pack) + '》</span></div>';
    }).join('');
  }
  ...
  function renderLifeMain() {
    var box = $("life-detail");
    /* 数据自愈：清理非法 key（如误塞进来的"收纳架"）、补齐缺失功能、保证选中项合法，并写回存储 */
    try { if (healLife()) Store.save(); } catch (e) { }
    var order = Store.data.life.order || [];
    /* 兜底：当前选择无效时自动重置为第一个可见功能，避免右侧长期空白 */
    if (!state.lifeSel || order.indexOf(state.lifeSel) < 0) state.lifeSel = order[0] || "";
    var key = state.lifeSel;
  ...
  /* 生活区数据自愈：清理非法 key（如误塞进生活区的"收纳架"等）、去重、补齐缺失功能、保证选中项合法。返回是否做了修改 */
  function healLife() {
    if (!Store || !Store.data || !Store.data.life) return false;
    var valid = LIFE_FEATS.map(function (x) { return x.key; });
    var order = Store.data.life.order, hidden = Store.data.life.hidden;
    if (!Array.isArray(order)) order = Store.data.life.order = [];
    if (!Array.isArray(hidden)) hidden = Store.data.life.hidden = [];
    var changed = false;
    /* 1) 去掉 order 里的非法 key（不在功能表里的一律剔除，避免整区空白） */
    var cleaned = order.filter(function (k) { return valid.indexOf(k) >= 0; });
    if (cleaned.length !== order.length) { changed = true; order = Store.data.life.order = cleaned; }
    /* 2) 去重 */
    var seen = {}, dedup = [];
    order.forEach(function (k) { if (!seen[k]) { seen[k] = 1; dedup.push(k); } });
    if (dedup.length !== order.length) { changed = true; order = Store.data.life.order = dedup; }
    /* 3) 把所有合法功能补回可见列表（除非用户主动隐藏） */
    valid.forEach(function (k) {
      if (order.indexOf(k) < 0 && hidden.indexOf(k) < 0) { order.push(k); changed = true; }
    });
    /* 4) 兜底：若仍为空，直接恢复默认 12 项 */
    if (!order.length) { Store.data.life.order = valid.slice(); changed = true; }
    /* 5) 确保当前选中项是合法的（否则整片空白） */
    if (valid.indexOf(state.lifeSel) < 0) { state.lifeSel = Store.data.life.order[0] || ""; changed = true; }
    return changed;
  }
  ...
  function init() {
    ...
    normalizeLifeSelection();
    /* 急救开关：地址栏末尾加 #fix-life 可强制把生活区重置为默认 12 个功能（解决任何数据损坏/旧缓存导致的空白） */
    if (location.hash && location.hash.indexOf("fix-life") >= 0) {
      try {
        Store.data.life.order = LIFE_FEATS.map(function (x) { return x.key; });
        var valid = LIFE_FEATS.map(function (x) { return x.key; });
        Store.data.life.hidden = (Store.data.life.hidden || []).filter(function (k) { return valid.indexOf(k) >= 0; });
        state.lifeSel = Store.data.life.order[0] || "";
        Store.save();
        setTimeout(function () { toast("✅ 生活区已强制修复为默认 12 个功能"); }, 300);
      } catch (e) { toast("修复失败：" + (e && e.message || e)); }
    }
    applyActiveBg();
    applyFont();
    applyMemoPriorityColors();
    ...
    for (var s = 0; s < subs.length; s++) {
      subs[s].addEventListener("click", function () {
        ...
        $("ent-archive").hidden = state.entSub !== "archive";
        if (state.entSub === "novel") renderEntList();
        else if (state.entSub === "insp") renderInspList();
        else if (state.entSub === "archive") renderArchive();
      });
    }
    ...
    /* 文包目录：单本/文包子胶囊切换与录入 */
    var archSubs = document.querySelectorAll("[data-arch-sub]");
    for (var as = 0; as < archSubs.length; as++) {
      archSubs[as].addEventListener("click", function () {
        archSub = this.getAttribute("data-arch-sub");
        for (var ak = 0; ak < archSubs.length; ak++) archSubs[ak].classList.toggle("active", archSubs[ak].getAttribute("data-arch-sub") === archSub);
        $("arch-single").hidden = archSub !== "single";
        $("arch-pack").hidden = archSub !== "pack";
        renderArchive();
      });
    }
    function saveSingleInput() {
      var raw = $("single-input").value; if (!raw.trim()) { toast("先粘点内容进来"); return; }
      try {
        if (!Store.data.ent) Store.data.ent = {};
        if (!Array.isArray(Store.data.ent.single)) Store.data.ent.single = [];
        var r = parseSingleInput(raw);
        var s = { id: uid(), raw: raw, title: r.title || "待整理", author: r.author || "", source: r.source || "", code: r.code || "", isBaidu: r.isBaidu, star: 0, date: todayStr() };
        Store.data.ent.single.unshift(s);
        var ok = Store.save();
        if (!ok) { toast("保存失败：存储空间可能已满"); return; }
        var savedRaw = raw;
        setTimeout(function () {
          if ($("single-input").value.trim() === savedRaw.trim()) {
            $("single-input").value = ""; $("single-preview").innerHTML = "";
          }
        }, 900);
        renderSingleList(); toast("已收下《" + s.title + "》（共 " + Store.data.ent.single.length + " 条）");
      } catch (err) {
        toast("保存失败：" + (err && err.message ? err.message : "未知错误"));
        console.error("saveSingleInput error", err);
      }
    }
    $("single-parse").onclick = saveSingleInput;
    /* 单本：粘贴即自动保存，零填写。不读 clipboardData，等浏览器写完 input 再读，避免时序/权限问题 */
    $("single-input").addEventListener("paste", function (e) {
      setTimeout(function () {
        renderSinglePreview();
        saveSingleInput();
      }, 50);
    ...
    function renderSinglePreview() {
      var raw = $("single-input").value; var box = $("single-preview");
      if (!box) return;
      if (!raw.trim()) { box.innerHTML = ""; return; }
      var r = parseSingleInput(raw);
      box.innerHTML = '<div class="card" style="background:#f9fbf8;margin-top:8px"><div style="font-size:12px;color:#8aa885;margin-bottom:4px">识别预览（粘贴后自动保存）</div>' +
        '<div class="it-title">《' + esc(r.title || "待整理") + '》</div>' +
        (r.author ? '<div class="it-body" style="font-size:12px;color:#6b8a6a">作者：' + esc(r.author) + '</div>' : '') +
        (r.isBaidu ? '<div class="it-body" style="font-size:12px;color:#6b8a6a">百度网盘' + (r.code ? ' · 提取码 ' + esc(r.code) : '') + '</div>' : '') +
        '</div>';
    }
    $("single-input").addEventListener("input", renderSinglePreview);
    $("single-list").addEventListener("click", function (e) {
      var starBtn = e.target.closest("[data-star]");
      if (starBtn) {
        var li = starBtn.closest("li"); var id = li.getAttribute("data-id");
        var s = Store.data.ent.single.filter(function (x) { return x.id === id; })[0]; if (!s) return;
        s.star = parseInt(starBtn.getAttribute("data-star"), 10); Store.save(); renderSingleList();
        return;
      }
      /* 删除按钮（包括折叠态头部）优先于折叠展开，避免点删除时误触发折叠 */
      var delBtn = e.target.closest('[data-act="del"]');
      if (delBtn) {
        var li = delBtn.closest("li"); var id = li.getAttribute("data-id");
        var s = Store.data.ent.single.filter(function (x) { return x.id === id; })[0]; if (!s) return;
        confirmDelete("删除收录", "删除《" + s.title + "》的收录？", function () { Store.data.ent.single = Store.data.ent.single.filter(function (x) { return x.id !== id; }); Store.save(); renderSingleList(); });
        return;
      }
      var head = e.target.closest('[data-act="toggle"]');
      if (head) {
        var li = head.closest("li"); var id = li.getAttribute("data-id");
        var s = Store.data.ent.single.filter(function (x) { return x.id === id; })[0]; if (!s) return;
        s._expanded = !s._expanded; Store.save(); renderSingleList();
        return;
      }
      ...
      if (act === "del") { confirmDelete("删除收录", "删除《" + s.title + "》的收录？", function () { Store.data.ent.single = Store.data.ent.single.filter(function (x) { return x.id !== id; }); Store.save(); renderSingleList(); }); }
    });
    $("pack-pick").onclick = function () { $("pack-file").click(); };
    $("pack-file").addEventListener("change", function () {
      var files = this.files; if (!files || !files.length) return;
      toast("正在解析 " + files.length + " 个压缩包…");
      if (!Store.data.ent) Store.data.ent = {};
      if (!Array.isArray(Store.data.ent.archive)) Store.data.ent.archive = [];
      var tasks = [];
      for (var fi = 0; fi < files.length; fi++) {
        (function (f) {
          tasks.push(ArchiveParser.readPackNames(f).then(function (items) {
            if (!items || !items.length) { toast("「" + f.name + "」里没找到小说文件（txt/epub…）"); return; }
            var its = items.map(function (it) { return { name: it.name, base: it.base, dir: it.dir, title: titleFromFilename(it.base), author: authorFromFilename(it.base) }; });
            Store.data.ent.archive.unshift({ id: uid(), name: f.name, size: f.size, items: its, date: todayStr() });
          }));
        })(files[fi]);
      }
      var self = this;
      Promise.all(tasks).then(function () {
        Store.save(); renderArchiveList(); self.value = "";
        toast("已归档 " + files.length + " 个文包，点卡片标题可折叠");
      }).catch(function (err) {
        console.error("解析压缩包失败", err);
        toast("解析失败：" + (err && err.message ? err.message : "压缩包格式可能不支持"));
        self.value = "";
      });
    });
    $("archive-search").addEventListener("input", function () { archiveReverseSearch(this.value); });
  ...
  /* 全局错误兜底：把运行时异常显示成提示条，方便排查"界面在但点了没反应"类问题 */
  var _errSeen = {};
  window.addEventListener("error", function (e) {
    var msg = (e && e.error && e.error.message) ? e.error.message : ((e && e.message) || "未知错误");
    console.error("[工作台运行时错误]", e && e.error);
    if (_errSeen[msg]) return; _errSeen[msg] = 1;
    try { toast("⚠️ 出错了：" + msg); } catch (_) {}
  });
